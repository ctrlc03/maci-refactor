...
contract Poll is Owned, Params, Utilities, EmptyBallotRoots {
    ...
    function deactivateKey(
        ...
        /// @notice we check that the deactivation period has not passed
        if (block.timestamp > extContracts.maci.deactivationPeriod()) revert DeactivationPeriodPassed();
        ...
        /// @notice check numDeactivatedKeys does not exceed max 
        unchecked {
            numMessages++;
            numDeactivatedKeys++;
        }
        ...
        emit PublishedMessage(_message, _encPubKey);
        emit AttemptedKeyDeactivation(_message, _encPubKey);
    }
    ...
    function generateNewKeyFromDeactivated(
        ...
        /// @todo check numGeneratedKeys does not exceed max
        unchecked {
            numMessages++;
            numGeneratedKeys++;
        }
        ...
        emit AttemptedKeyGeneration(_message, _encPubKey, newStateIndex);
