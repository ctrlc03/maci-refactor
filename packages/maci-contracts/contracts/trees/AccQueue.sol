 ...
 */
abstract contract AccQueue is Owned, Hasher {
    ...
    uint256 public hashLength;

    // hashLength ** subDepth
    uint256 public subTreeCapacity;
    ...
    /// @notice whether the contract was initialized or not
    ...
    /**
     * @notice Initializes the contract (to be used with clones)
     * @param _subDepth The depth of each subtree.
     * @param _hashLength The number of elements per hash operation. Should be
     *                   either 2 (for binary trees) or 5 (quinary trees).
    ...
    function initialize(
        ...
        if (_subDepth == 0) revert SubDepthTooSmall();
        if (_subDepth > MAX_DEPTH) revert SubDepthTooLarge();
        if (_hashLength != 2 && _hashLength != 5) revert HashLengthNotSupported();

        /// @dev set to true so it cannot be called again 
        isInitialized = true;

        /// @dev overwrite the owner to work with clones
        owner = _owner;
     ...
     * @param _level The specified level
     * @return uint256 The zero leaf at the specified level.
     */
     ...
     * @return uint256 The index of the leaf.
     */
     ...
     * @param _subRoot The subroot to insert.
     */
     ...
     * merged together.
     * @return uint256 The height of the tree.
     */
    ...
    function merge(uint256 _depth) public onlyOwner returns (uint256) {
        ...
        if (_depth == 0) revert DepthTooSmall();
        // Check the depth
        ...
        if (_depth < srtDepth) revert DepthSmallerThenSrt();
    ...
    function getSubRoot(uint256 _index) public view returns (uint256) {
        if (currentSubtreeIndex <= _index) revert IndexDoesNotExist();
...
contract AccQueueBinary0 is AccQueueBinary, MerkleBinary0 {
    constructor() AccQueueBinary() payable {}
...
contract AccQueueQuinary0 is AccQueueQuinary, MerkleQuinary0 {
    constructor() AccQueueQuinary() payable {}
    function getZero(uint256 _level) internal view override returns (uint256) { return zeros[_level]; }
}
